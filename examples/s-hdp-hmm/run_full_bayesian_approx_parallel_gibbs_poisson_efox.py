...
## nohup python run_full_bayesian_approx_parallel_gibbs_poisson_efox.py 1 20 2 'prior' i01_maze15_2d_data_100ms_sample_trials ./ 200 &
...
prior_params ={'lam_a_pri':1.0, 'lam_b_hyper_pri_shape':1.0, 'lam_b_hyper_pri_rate':1.0, 'm_multi':m_multi}; ## gamma prior parameters for poisson observations firing rate, can adjust it based on data
...
for it in range(iters):
    ...
    ## compute loglik
    if it%10 == 0:
        loglik_test = compute_log_marginal_lik_poisson_fmp(L, pi_bar, pi_init, suff_stats,yt_test_ls,n_cores);
